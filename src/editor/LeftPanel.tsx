...
function Templates() {
  ...
  const project=useEditorStore(state=>state.project);const updatePage=useEditorStore(state=>state.updatePage);const page=getActivePage(project);
  function insert(html:string,css?:string){window.dispatchEvent(new CustomEvent('webdev-insert-widget',{detail:html}));if(css&&!page.css.includes(css))updatePage({css:`${page.css}\n${css}`} ,'Aplicar estilo do modelo')}
  const templates=[
  ...
  return <div className="templates-panel"><h3>Modelos e componentes</h3>{custom.map((template,index)=><button className="template-card" key={template.id} onClick={()=>insert(template.html,template.css)}><span className={`template-thumb template-${index%4}`}/><strong>{template.name}</strong><small>{template.global?'Componente global':'Modelo personalizado'}</small></button>)}{templates.map(({name,text,html},index)=><button className="template-card" key={name} onClick={()=>insert(html)}><span className={`template-thumb template-${index}`}/><strong>{name}</strong><small>{text}</small></button>)}</div>;
}
